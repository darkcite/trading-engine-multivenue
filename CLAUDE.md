...
- **NEXT = M1** of `docs/mvp-completion-plan.md` (Stage 2.5): universe config file + Binance multi-symbol spot/USDS-M futures + Polymarket multi-market (M1 kickoff prompt at the end of `docs/phase-8h-progress.md`) — then M2…M6 (options ×3 venues → always-on data ops → shadow-P&L → semi-manual loop on real data → soak + sign-off). **M-phase session notes go to `docs/mvp-progress.md`** (create on first entry); the 8h progress file is CLOSED history. Plan §9 (data storage: PMLR canonical, candles.db keyed venue+descriptor, fetch 1m/1h/1d + derive, backtest reads PMLR only) is BINDING.

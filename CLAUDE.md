...
- **8h sessions:** H0 design LOCKED (**H-D1…H-D8**, all option (a)) · H1 harness substrate CLOSED (`3ad40a9`) · H2 §4 fill/fee/latency model CLOSED (`1ed6017`) · H3 data_fetcher §6 CLOSED (`76680db`) · H4 strategist §7 + §8.1/§8.2 auto-promotion CLOSED (`7bd0e42`) · H5 rollback §8.3/§8.4 CLOSED (`0e47429`) — `monitor.py` + `ResearchCycle` own the walk-forward monitor (trailing 24 h / 6 h floor, run-granular window, `split=0/100`, net ≤ −$100 OR dd ≥ $200 ⇒ Disable-5 THEN frozen restage/commit of the prior; no-prior ⇒ disable-only + dark guard; report-clobber protection via active-copy scoring; events-ledger active resolution; §7.1 performance seam wired). The full autonomous loop is CODE-COMPLETE.
- **H6a CLOSED (this commit) — operator-rescoped; PHASE STILL OPEN, H6b OWED.** H6a demonstrated live (no Anthropic key ⇒ semi-manual ruling): final gates all green; paper boot on the Fed −25bps Sept market; the REAL harness refusing a real-capture ruleset (exit 3 — and the standing finding that no current real capture can satisfy `min_trading_days=2`); a Fable-5-authored ruleset (`d8aea5f4…`, in-session authorship) passing gates on a §8.5-sanctioned crafted capture and promoted through the FROZEN verb lane onto the LIVE engine (staged/committed 0→1, vm_rows_active 2, table_epoch 1, vm_fires 1 — trading in paper); audit-replay rendering Stage seq=16 / Commit seq=18; the owed H3 §6.1 fetch smoke CLOSED (live Gamma, market-map bootstrapped + additively refreshed). Rollback + §8.1 auto-promotion NOT run (operator ruling) ⇒ the §12 exit criteria are NOT yet fully demonstrated.
- **NEXT SESSION = H6b.** The verbatim H6b kickoff prompt is the last section of `docs/phase-8h-progress.md` — paste it into the fresh session. Scope: one REAL keyed Fable-5 serve cycle (§8.1 auto-promotion observed live, budget-capped) + the §8.5 forced-underperformance rollback observed live + audit-replay + the phase-closing entry. PREREQS: `ANTHROPIC_API_KEY` in `.env` (operator's hand), HMAC key provisioning, and (if /tmp cleared) a supersede re-stage of `d8aea5f4…` from `~/multivenue/worker/demo-h6a/` so the prior binds. **No new feature code.** The Stage-2 completion notification fires at H6b close.

...
- **STAY-GREENS (2026-09-03, VT4 battery): nextest 1472 · release alloc 39/39 0 B/op · worker pytest 618 (frozen 202 inside) · `make lint` green · `make license-check` green.** Fuzz = VM2-V4's standing ≥300 s record. Two known launchd-context test flakes (isolation-disproven, recorded in the archived vm2-plan §8 close entry): `ai_exec_on_ai_is_zero_alloc` (debug profile) and `scrape_hammer_all_succeed_without_conn_errors` — rerun in isolation before believing a red.

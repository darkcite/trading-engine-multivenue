...
- **ACTIVE AUTHORITY: `docs/stage2-finish-plan.md` (2026-08-29 operator ruling — read it FIRST).** Single WS0–WS13 numbering. **WS2–WS12 CODED + the WS13 GATES PHASE GREEN + ALL COMMITTED (2026-08-29 session, commits `a9f5b4b`…`46f9610` + CLAUDE.md close):** WS2 drop/establishment semantics (OKX/Deribit/Bybit) · WS3–WS8 venue expansions · WS9 Bybit sixth venue (data-only, lane 5/venue 6) · WS11 worker refdata/funding/D5 fold-ins · WS12 `make lint` green. Gates run on operator order: **nextest 1316 · alloc 38 0 B/op · pytest 477 (frozen 202 inside) · fuzz 5×301 s clean · fmt/lint/license green — the NEW stay-greens 1316/38/477**; 6 regression fixes landed during the run (WS9 seventh-venue desyncs: `venue_records[6]` OOB ×2 files, binance `parse_filters` Truncated-vs-BadRow, 3 test/alias fixes) — record in the plan's WS13 entry. **REMAINING: (a) WS10 design review (`docs/ws10-engine-plumbing-design.md`, D-A1…D-B3) then its CODE; (b) WS13 LIVE phase = plan steps 3–7 (reboot standing lane on the new binary, live smoke all venues incl. Bybit, one settlement-cycle verification, C6/M3 close + D1 fetch, 7-day full-scope soak, then declare Stage 2 FINISHED); (c) WS2 §5.4 chronic-OKX-churn named-cause fix, gated on WS13's first live hour.** The earlier remediation commits (`24d545a`…`09a7bbb`) are live-UNTESTED until WS13's live phase; WS0 operator levers unchanged (restart-lane revive: `echo 19700101 > ~/multivenue/state/last-restart-utc-0000`; #7b arms at next boot unless disarmed). Background: `docs/capture-continuity-outage-2026-08-27.md` → `docs/capture-remediation-plan-2026-08-28.md` §12 → `docs/prompts/remediation-run-phase.md` (folds into WS13).

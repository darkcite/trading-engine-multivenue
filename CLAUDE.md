...
- **VM2 (ruleset-VM v2) V0–V7 DONE + V8 PREP DONE (2026-08-30; authority `docs/vm2-plan.md`, §8 log + §9 runbook — read its "V8 EXECUTION RESUME POINT" entry FIRST if resuming VM2 work).** Commits `70bc59b` V5 · `342eccc` V6 · `6cc1ba5` V7 · `2a283a5` V8-prep. The general grammar (17 features / 4 combines / positions / groups / holds / confirms), engine opt+event+depth lanes, seeds (kinds 10/11), multi-channel backtest with D-7 mark-fill + dead-descriptor drop, depth_digest/coverage_audit/channel_map, per-host candle budgets (live starvation fix), parity comparator, 6+1 authored artifacts real-backtested through the frozen verb. Stay-greens **1420 / 39 / 550** (553 with release binary on PATH). **V8 execution is TIME-GATED (operator ruling): xv-v2 `33e91345…` reruns ~Sep-1/2 → stage+commit → 48 h parity → per-family cron bootout on explicit order; crons keep carrying until then. V9 closes.** The M5 crons (com.multivenue.carry/xv) stay RUNNING.

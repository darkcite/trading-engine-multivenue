...
# (default: the exec lane, core-net and ingress-binance) that has NO
# `COPY:` marker within
...
#     auditor's.
#
#   scripts/copy-audit.sh                       # exec lane + core-net + ingress-binance
...
if [ "$#" -eq 0 ]; then
    set -- crates/exec-router crates/exec-hyperliquid crates/signer-eip712 \
           crates/clob-dispatcher crates/core-net crates/ingress-binance
fi

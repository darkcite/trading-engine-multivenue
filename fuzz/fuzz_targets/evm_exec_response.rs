...
//! classifier, and (H9) the `eth_call` word. Untrusted network input
//! on the arm's thread: no panic,
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = exec_hyperevm::rpc::scan_receipt(data, id, &mut rc);
    let _ = exec_hyperevm::rpc::scan_word(data, id);

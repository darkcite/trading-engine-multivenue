...
//! Fuzz target: arbitrary bytes → `ingress_okx::walk_book_levels`
//! (WS10-B — the `books` level walker feeding the depth ladder).
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = ingress_okx::walk_book_levels(data, &mut ladder);

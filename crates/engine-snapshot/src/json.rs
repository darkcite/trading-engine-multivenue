...
pub fn encode_state_json(s: &EngineSnapshot, dst: &mut [u8]) -> Result<usize, JsonOverflow> {
    ...
    // E7: the ledger's alarms and the live arm's numbers. Additive.
    c.key("ledger_fills_unbound");
    c.u64(ex.ledger_fills_unbound);
    c.key("ledger_sells_below_zero");
    c.u64(ex.ledger_sells_below_zero);
    c.key("ledger_resting_full");
    c.u64(ex.ledger_resting_full);
    c.key("ledger_resting_ambiguous");
    c.u64(ex.ledger_resting_ambiguous);
    c.key("arm_fills_booked");
    c.u64(ex.arm_fills_booked);
    c.key("arm_fills_dropped");
    c.u64(ex.arm_fills_dropped);
    c.key("arm_fills_unresolved");
    c.u64(ex.arm_fills_unresolved);
    c.key("arm_sent_unanswered");
    c.u64(ex.arm_sent_unanswered);
    c.key("arm_recon_ok");
    c.u64(ex.arm_recon_ok);
    c.key("arm_recon_failed");
    c.u64(ex.arm_recon_failed);
    c.key("arm_recon_drift_legs");
    c.u64(ex.arm_recon_drift_legs);
    c.key("arm_recon_unseen_legs");
    c.u64(ex.arm_recon_unseen_legs);
    c.key("arm_sweep_left");
    ...
    // never said — WHICH contract, at what strike, with what on each
    // leg. A naked hedge reads as `opt_qty_1e6 == 0` with
    // `perp_qty_1e6 != 0`; a stuck leg as a non-zero `opt_oid`/
    // `hedge_oid` that does not clear.
    let vr = &s.vrp;
    let vv = &vr.view;
    let vc = &vr.counters;
    c.key("vrp");
    c.put(b"{\"configured\":");
    c.u64(u64::from(vv.configured));
    c.key("hash");
    c.hex(&vv.hash);
    c.key("state_epoch");
    c.u64(vv.state_epoch);
    c.key("regime_offset_1e9");
    c.i64(vv.regime_offset_1e9);
    c.key("last_settle_value_1e6");
    c.i64(vv.last_settle_value_1e6);
    c.key("campaign");
    c.put(b"{\"expiry_ns\":");
    c.u64_str(vv.expiry_ns);
    c.key("sym");
    c.u64(u64::from(vv.selected_sym));
    c.key("strike_1e6");
    c.i64(vv.strike_1e6);
    c.key("right");
    c.u64(u64::from(vv.right));
    c.key("side");
    c.i64(i64::from(vv.side));
    c.key("opt_qty_1e6");
    c.i64(vv.opt_qty_1e6);
    c.key("perp_qty_1e6");
    c.i64(vv.perp_qty_1e6);
    c.key("entry_done");
    c.u64(u64::from(vv.entry_done));
    c.put(b"}");
    c.key("pending");
    c.put(b"{\"opt_oid\":");
    c.u64_str(vv.opt_oid);
    c.key("hedge_oid");
    c.u64_str(vv.hedge_oid);
    c.put(b"}");
    c.key("decisions");
    c.u64(vc.decisions);
    c.key("decisions_late");
    c.u64(vc.decisions_late);
    c.key("holds");
    c.u64(vc.holds);
    c.key("holds_side");
    c.u64(vc.holds_side);
    c.key("holds_cost");
    c.u64(vc.holds_cost);
    c.key("entries_submitted");
    c.u64(vc.entries_submitted);
    c.key("entries");
    c.u64(vc.entries);
    c.key("entries_unfilled");
    c.u64(vc.entries_unfilled);
    c.key("entry_maker_submitted");
    c.u64(vc.entry_maker_submitted);
    c.key("entry_crossed");
    c.u64(vc.entry_crossed);
    c.key("entry_cost_refused");
    c.u64(vc.entry_cost_refused);
    c.key("hedges");
    c.u64(vc.hedges);
    c.key("hedge_unfilled");
    c.u64(vc.hedge_unfilled);
    c.key("hedge_crossed");
    c.u64(vc.hedge_crossed);
    c.key("hedge_abandoned");
    c.u64(vc.hedge_abandoned);
    c.key("fills");
    c.u64(vc.fills);
    c.key("fills_ignored");
    c.u64(vc.fills_ignored);
    c.key("settlements");
    c.u64(vc.settlements);
    c.key("settled_itm");
    c.u64(vc.settled_itm);
    c.key("settled_otm");
    c.u64(vc.settled_otm);
    c.key("settled_unpriced");
    c.u64(vc.settled_unpriced);
    c.key("settle_index_fallback");
    c.u64(vc.settle_index_fallback);
    c.key("iv_median_fallback");
    c.u64(vc.iv_median_fallback);
    c.key("records_ignored");
    c.u64(vc.records_ignored);
    c.key("stale_skips");
    c.u64(vc.stale_skips);
    c.key("no_bounds");
    c.u64(vc.no_bounds);
    c.key("no_selection");
    c.u64(vc.no_selection);
    c.key("select_scans");
    c.u64(vc.select_scans);
    c.key("caps_rejected");
    c.u64(vc.caps_rejected);
    c.key("regime_blocked");
    c.u64(vc.regime_blocked);
    c.key("regime_exits");
    c.u64(vc.regime_exits);
    c.key("killed");
    c.u64(vc.killed);
    c.key("qlike_iv_1e6");
    c.i64(vc.qlike_iv_1e6);
    c.key("qlike_har_1e6");
    c.i64(vc.qlike_har_1e6);
    c.key("qlike_har_beats_iv");
    c.u64(vc.qlike_har_beats_iv);
    c.put(b"}");
    ...
    // --- har (HAR H3.5) ---
    //
    // Additive: `"v":1` stays. One row per long-tenor series: at the

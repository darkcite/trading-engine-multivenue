...
//! hash of what went over the wire. Everything else is scripted, and
//! most scripts are failures — the failures are what the arm exists to
...
struct Node {
    ...
    accounts: HashMap<[u8; 20], (u64, u64, u128)>,
    script: VecDeque<SendScript>,
    ...
    match method.as_str() {
        ...
        "eth_sendRawTransaction" => {
            ...
            match node
                .script
                .pop_front()
                .expect("a send the test did not script")
            {
                SendScript::Accept {
                    from,
                    to,
        ...
        "eth_getTransactionReceipt" => {
            ...
            match node.txs.get(&h) {
                Some(t) if t.mined => {
                    ...
                    // top level only.
                    result(format!(
                        r#"{{"type":"0x2","status":"0x{:x}","logs":[{{"blockNumber":"0x1","transactionHash":"0x{}"}}],"transactionHash":"0x{}","blockNumber":"0x3e8","gasUsed":"0x1d4c0","effectiveGasPrice":"0x5f5e100","from":"0x{}","to":{to},"contractAddress":{c}}}"#,
                        t.status,
                        "00".repeat(32),
                        hex(&h),
                        hex(&t.from),
...
fn boot_node(node: Node) -> (u16, Arc<ClientConfig>, Arc<Mutex<Node>>) {
    ...
    thread::spawn(move || {
        for conn in listener.incoming() {
            ...
            while let Some(req) = read_request(&mut stream) {
                let reply = handle(&mut shared.lock().unwrap(), &req);
                match reply {
                    Reply::Body(body) => {
                        let head = format!(

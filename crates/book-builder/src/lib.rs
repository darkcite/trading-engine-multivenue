...
impl TopOfBook {
    ...
    /// Apply a tick. Ignores out-of-order updates and ticks for
    /// other symbols.
    #[inline]
...
impl<const N: usize> MultiBook<N> {
    ...
    /// `sym`; silently drops ticks for untracked symbols (the same
    /// stream may feed multiple consumers).
    ///
    ...
    /// Returns `false` if `idx` is out of range or the slot's
    /// symbol no longer matches the tick; the caller should fall
    /// back to `apply` in that case.
    #[inline(always)]
    pub fn apply_at(&mut self, idx: u32, tick: &Tick) -> bool {
        let n = self.count as usize;
        let i = idx as usize;
        if i >= n {
            return false;

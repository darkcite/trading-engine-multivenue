...
//! [`Signal`] popped from the ring with the expected block number.
...
#[test]
fn rpc_tls_loopback_yields_expected_signal() {
    let cert = make_cert();
    ...
    let server = thread::spawn(move || {
        ...
        // newHead notification — block 0x1234 = 4660, ts 0x5faa, gas
        // 0x7a1200.
        let body = br#"{"jsonrpc":"2.0","method":"eth_subscription","params":{"subscription":"0xab","result":{"number":"0x1234","timestamp":"0x5faa","gasUsed":"0x7a1200","hash":"0xdeadbeef"}}}"#;
        let frame = build_unmasked_text_frame(body);
        stream.write_all(&frame).expect("write notification");
    ...
    let deadline = Instant::now() + Duration::from_secs(5);
    let mut got: Option<Signal> = None;
    ...
    assert_eq!(number, 0x1234, "block number");
    assert_eq!(ts, 0x5faa, "timestamp");
    assert_eq!(gas, 0x7a1200, "gas used");
}

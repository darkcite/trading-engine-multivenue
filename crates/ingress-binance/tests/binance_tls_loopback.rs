//! Integration test: Binance bookTicker ingress against a real
//! `127.0.0.1` TLS server with a self-signed cert. Mirror of the
//! `polymarket_tls_loopback` test — same scaffolding, different
//! scripted frame.
...
#[test]
fn binance_tls_loopback_yields_expected_tick() {
    let cert = make_cert();
    ...
    // Signal-driven sync: the client thread flips `client_done`
    // once it has popped the expected Tick. The server thread
    // holds the connection open until the flag flips or until a
    // bounded 5 s deadline elapses. No `thread::sleep` mid-frame.
    ...
    let server = thread::spawn(move || {
        ...
        // Canonical Binance bookTicker frame.
        let body = br#"{"u":400900217,"s":"BTCUSDT","b":"65000.01","B":"1.234","a":"65000.55","A":"0.987"}"#;
        let frame = build_unmasked_text_frame(body);
        stream.write_all(&frame).expect("write frame");
        stream.flush().expect("flush stream");

        // Wait for the client to confirm receipt, or fall through
        // on a 5 s deadline. Sleep is bounded and short (1 ms) so
        // shutdown is prompt once the flag flips.
    ...
    assert_eq!(tick.ask_px.raw(), 65_000_550_000);
}

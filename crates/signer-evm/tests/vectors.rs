...
use signer_evm::{signed_hex_len, tx_encode_signed_hex, tx_hash, tx_sign, tx_signing_digest, y_parity_from_v, Eip1559Tx, EvmTxErr};
...
fn wants() -> Vec<Want> {
    ...
    for line in VECTORS.lines() {
        ...
        match f[1] {
            "digest" => cur = Some(Want { digest: f[2].into(), raw: String::new(), hash: String::new(), v: 0, r: String::new(), s: String::new() }),
...
fn eip1559_known_answer_vectors() {
    ...
    let big: Vec<u8> = (0..256u32).map(|x| x as u8).chain((0..44u32).map(|x| x as u8)).collect();
    let mut b80 = vec![0x80u8];
    b80.extend(std::iter::repeat_n(0u8, 54));
    let txs = [
        Eip1559Tx { chain_id: 998, nonce: 0, max_priority_fee_per_gas: 0, max_fee_per_gas: 0, gas_limit: 21_000, to: addr("000000000000000000000000000000000000dead"), value: 0, data: &[] },
        Eip1559Tx { chain_id: 998, nonce: 7, max_priority_fee_per_gas: 1_000_000_000, max_fee_per_gas: 3_000_000_000, gas_limit: 250_000, to: addr("5555555555555555555555555555555555555555"), value: 1, data: &erc20 },
        Eip1559Tx { chain_id: 999, nonce: 0x12_3456_7890, max_priority_fee_per_gas: 7, max_fee_per_gas: (1u128 << 100) + 12_345, gas_limit: 3_000_000, to: addr("6c9a33e3b592c0d65b3ba59355d5be0d38259285"), value: (1u128 << 127) - 1, data: &big },
        Eip1559Tx { chain_id: 998, nonce: 127, max_priority_fee_per_gas: 128, max_fee_per_gas: 255, gas_limit: 127, to: addr("0000000000000000000000000000000000000001"), value: 0, data: &[0x7f] },
        Eip1559Tx { chain_id: 998, nonce: 1, max_priority_fee_per_gas: 1, max_fee_per_gas: 1, gas_limit: 1, to: addr("0000000000000000000000000000000000000001"), value: 0, data: &b80 },
    ];
    let wants = wants();
    ...
    for (i, (tx, w)) in txs.iter().zip(wants.iter()).enumerate() {
        ...
        assert_eq!(std::str::from_utf8(&buf[..n]).unwrap(), format!("0x{}", w.raw), "V{i} raw");
...
fn y_parity_is_zero_or_one_and_bad_v_is_refused() {
    ...
    let tx = Eip1559Tx { chain_id: 998, nonce: 0, max_priority_fee_per_gas: 0, max_fee_per_gas: 0, gas_limit: 21_000, to: [0; 20], value: 0, data: &[] };
    let sk = key();
    let mut k = 0u64;
    while k < 64 {
    ...
    for bad in [0u8, 1, 26, 29, 35, 36, 255] {
        sig[64] = bad;
        let mut buf = [0u8; 512];
        assert_eq!(tx_encode_signed_hex(&tx, &sig, &mut buf), Err(EvmTxErr::BadSignature));
...
fn a_short_buffer_writes_nothing() {
    let tx = Eip1559Tx { chain_id: 998, nonce: 0, max_priority_fee_per_gas: 0, max_fee_per_gas: 0, gas_limit: 21_000, to: [0; 20], value: 0, data: &[1, 2, 3] };
    let sig = tx_sign(&tx, &key()).unwrap();
    let need = signed_hex_len(&tx, &sig).unwrap();
    let mut buf = vec![0xAAu8; need - 1];
    assert_eq!(tx_encode_signed_hex(&tx, &sig, &mut buf), Err(EvmTxErr::BufferTooSmall));
    assert!(buf.iter().all(|&b| b == 0xAA), "a refused render must not touch the buffer");
}
